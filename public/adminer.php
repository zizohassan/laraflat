    ...
    if (get_magic_quotes_gpc()) {
        while (list($y, $X) = each($Nf)) {
            foreach ($X
                ...
                } else$Nf[$y][stripslashes($Id)] = ($Jc ? $W : stripslashes($W));
            }
        }
    }
}
...
function
optionlist($Ye, $zg = null, $bi = false)
{
    $J = "";
    foreach ($Ye
             as $Id => $W) {
        ...
        foreach ($Ze
                 as $y => $X) $J .= '<option' . ($bi || is_string($y) ? ' value="' . h($y) . '"' : '') . (($bi || is_string($y) ? (string)$y : $X) === $zg ? ' selected' : '') . '>' . h($X);
...
function
html_select($C, $Ye, $Y = "", $Re = true, $Qd = "")
{
    if ($Re) return "<select name='" . h($C) . "'" . (is_string($Re) ? ' onchange="' . h($Re) . '"' : "") . ($Qd ? " aria-labelledby='$Qd'" : "") . ">" . optionlist($Ye, $Y) . "</select>";
    $J = "";
    foreach ($Ye
             as $y => $X) $J .= "<label><input type='radio' name='" . h($C) . "' value='" . h($y) . "'" . ($y == $Y ? " checked" : "") . ">" . h($X) . "</label>";
    return $J;
}

function
select_input($Ia, $Ye, $Y = "", $_f = "")
{
...
function
print_fieldset($t, $Xd, $mi = false, $Se = "")
{
...
function
get_rows($H, $h = null, $n = "<p class='error'>")
{
    global $g;
    $tb = (is_object($h) ? $h : $g);
    $J = array();
    ...
    } elseif (!$I && !is_object($h) && $n && defined("PAGE_HEADER")) echo $n . error() . "\n";
    return $J;
}
...
where($Z, $p = array())
{
    ...
    foreach ((array)$Z["where"] as $y => $X) {
        ...
        if ($x == "sql" && preg_match('~char|text~', $p[$y]["type"]) && preg_match("~[^ -@]~", $X)) $J[] = "$e = " . q($X) . " COLLATE " . charset($g) . "_bin";
    }
...
convert_fields($f, $p, $M = array())
{
    ...
    foreach ($f
        ...
        if ($Fa) $J .= ", $Fa AS " . idf_escape($y);
    }
    return $J;
}

function
cookie($C, $Y, $ae = 2592000)
{
    global $ba;
    return
        header("Set-Cookie: $C=" . urlencode($Y) . ($ae ? "; expires=" . gmdate("D, d M Y H:i:s", time() + $ae) . " GMT" : "") . "; path=" . preg_replace('~\\?.*~', '', $_SERVER["REQUEST_URI"]) . ($ba ? "; secure" : "") . "; HttpOnly; SameSite=lax", false);
}
...
function
query_redirect($H, $A, $pe, $Xf = true, $vc = true, $Cc = false, $sh = "")
{
    global $g, $n, $b;
    if ($vc) {
        $Sg = microtime(true);
    ...
    $Qg = "";
    if ($H) $Qg = $b->messageQuery($H, $sh);
    if ($Cc) {
        $n = error() . $Qg;
        return
            false;
    }
    if ($Xf) redirect($A, $pe . $Qg);
    return
        true;
}
...
function
apply_queries($H, $T, $rc = 'table')
{
    foreach ($T
             as $R) {
        if (!queries("$H " . $rc($R))) return
            false;
    }
    return
        true;
...
get_file($y, $Jb = false)
{
    ...
    foreach ($Hc["error"] as $y => $n) {
        ...
        if ($Jb) {
            $Sg = substr($vb, 0, 3);
            if (function_exists("iconv") && preg_match("~^\xFE\xFF|^\xFF\xFE~", $Sg, $dg)) $vb = iconv("utf-16", "utf-8", $vb); elseif ($Sg == "\xEF\xBB\xBF") $vb = substr($vb, 3);
            $J .= $vb . "\n\n";
        } else$J .= $vb;
    }
    return $J;
...
input($o, $Y, $r)
{
    ...
    if ($gg && !$_POST["save"]) $r = null;
    $Xc = (isset($_GET["select"]) || $gg ? array("orig" => 'original') : array()) + $b->editFunctions($o);
    $Ia = " name='fields[$C]'";
    if ($o["type"] == "enum") echo
        nbsp($Xc[""]) . "<td>" . $b->editInput($_GET["edit"], $o, $Ia, $Y); else {
        $Kc = 0;
        foreach ($Xc
                 as $y => $X) {
            if ($y === "" || !$X) break;
        ...
        if ($xd != "") echo $xd; elseif (preg_match('~bool~', $o["type"])) echo "<input type='hidden'$Ia value='0'>" . "<input type='checkbox'" . (in_array(strtolower($Y), array('1', 't', 'true', 'y', 'yes', 'on')) ? " checked='checked'" : "") . "$Ia value='1'>";
        elseif ($o["type"] == "set") {
            preg_match_all("~'((?:[^']|'')*)'~", $o["length"], $he);
            foreach ($he[1] as $s => $X) {
                $X = stripcslashes(str_replace("''", "'", $X));
        ...
        } elseif (preg_match('~blob|bytea|raw|file~', $o["type"]) && ini_bool("file_uploads")) echo "<input type='file' name='fields-$C'$Re>";
        elseif (($qh = preg_match('~text|lob~', $o["type"])) || preg_match("~\n~", $Y)) {
            if ($qh && $x != "sqlite") $Ia .= " cols='50' rows='12'"; else {
                $L = min(12, substr_count($Y, "\n") + 1);
                $Ia .= " cols='30' rows='$L'" . ($L == 1 ? " style='height: 1.2em;'" : "");
            }
            echo "<textarea$Ia>" . h($Y) . '</textarea>';
        } elseif ($r == "json" || preg_match('~^jsonb?$~', $o["type"])) echo "<textarea$Ia cols='50' rows='12' class='jush-js'>" . h($Y) . '</textarea>';
        else {
            ...
            if ($x == 'sql' && $g->server_info >= 5.6 && preg_match('~time~', $o["type"])) $oe += 7;
...
function
dump_headers($md, $ye = false)
{
    global $b;
    $J = $b->dumpHeaders($md, $ye);
    $lf = $_POST["output"];
    if ($lf != "text") header("Content-Disposition: attachment; filename=" . $b->dumpFilename($md) . ".$J" . ($lf != "file" && !preg_match('~[^0-9a-z]~', $lf) ? ".$lf" : ""));
    session_write_close();
    ob_flush();
    flush();
    return $J;
...
function
select_value($X, $_, $o, $rh)
{
    global $b, $ba;
    if (is_array($X)) {
        $J = "";
        foreach ($X
                 as $Id => $W) $J .= "<tr>" . ($X != array_values($X) ? "<th>" . h($Id) : "") . "<td>" . select_value($W, $_, $o, $rh);
        return "<table cellspacing='0'>$J</table>";
    }
    if (!$_) $_ = $b->selectLink($X, $o);
    if ($_ === null) {
        if (is_mail($X)) $_ = "mailto:$X";
        if ($Pf = is_url($X)) $_ = (($Pf == "http" && $ba) || preg_match('~WebKit|Firefox~i', $_SERVER["HTTP_USER_AGENT"]) ? $X : "https://www.adminer.org/redirect/?url=" . urlencode($X));
    }
    $J = $b->editVal($X, $o);
    if ($J !== null) {
        if ($J === "") $J = "&nbsp;"; elseif (!is_utf8($J)) $J = "\0";
        elseif ($rh != "" && is_shortable($o)) $J = shorten_utf8($J, max(0, +$rh));
        else$J = h($J);
    }
...
edit_form($a, $p, $K, $Wh)
{
    ...
    if ($K === false) echo "<p class='error'>" . 'No rows.' . "\n";
...
';
    if (!$p) echo "<p class='error'>" . 'You have no privileges to update this table.' . "\n"; else {
        ...
        foreach ($p
            ...
            if ($Kb === null) {
                $Kb = $o["default"];
                if ($o["type"] == "bit" && preg_match("~^b'([01]*)'\$~", $Kb, $dg)) $Kb = $dg[1];
            }
...
if (extension_loaded('pdo')) {
        ...
        PDO
    {
        ...
        __construct()
        {
            ...
            if ($Bf !== false) unset($b->operators[$Bf]);
        }
...
Min_SQL
{
    ...
    function
    select($R, $M, $Z, $ad, $af = array(), $z = 1, $E = 0, $Jf = false)
    {
        global $b, $x;
        $Cd = (count($ad) < count($M));
        $H = $b->selectQueryBuild($M, $Z, $ad, $af, $z, $E);
        if (!$H) $H = "SELECT" . limit(($_GET["page"] != "last" && +$z && $ad && $Cd && $x == "sql" ? "SQL_CALC_FOUND_ROWS " : "") . implode(", ", $M) . "\nFROM " . table($R), ($Z ? "\nWHERE " . implode(" AND ", $Z) : "") . ($ad && $Cd ? "\nGROUP BY " . implode(", ", $ad) : "") . ($af ? "\nORDER BY " . implode(", ", $af) : ""), ($z != "" ? +$z : null), ($E ? $z * $E : 0), "\n");
        $Sg = microtime(true);
        $J = $this->_conn->query($H);
        if ($Jf) echo $b->selectQuery($H, format_time($Sg));
        return $J;
    }
...
if (isset($_GET["sqlite"]) || isset($_GET["sqlite2"])) {
    $Ef = array((isset($_GET["sqlite"]) ? "SQLite3" : "SQLite"), "PDO_SQLite");
    ...
    fields($R)
    {
        ...
        foreach (get_rows("PRAGMA table_info(" . table($R) . ")") as $K) {
            ...
            $U = strtolower($K["type"]);
            $Kb = $K["dflt_value"];
            $J[$C] = array("field" => $C, "type" => (preg_match('~int~i', $U) ? "integer" : (preg_match('~char|clob|text~i', $U) ? "text" : (preg_match('~blob~i', $U) ? "blob" : (preg_match('~real|floa|doub~i', $U) ? "real" : "numeric")))), "full_type" => $U, "default" => (preg_match("~'(.*)'~", $Kb, $B) ? str_replace("''", "'", $B[1]) : ($Kb == "NULL" ? null : $Kb)), "null" => !$K["notnull"], "privileges" => array("select" => 1, "insert" => 1, "update" => 1), "primary" => $K["pk"],);
            if ($K["pk"]) {
                if ($Hf != "") $J[$Hf]["auto_increment"] = false; elseif (preg_match('~^integer$~i', $U)) $J[$C]["auto_increment"] = true;
                $Hf = $C;
            }
        }
    ...
    indexes($R, $h = null)
    {
        ...
        foreach (get_rows("PRAGMA index_list(" . table($R) . ")", $h) as $K) {
            ...
            if (!$J[""] || $v["type"] != "UNIQUE" || $v["columns"] != $J[""]["columns"] || $v["descs"] != $J[""]["descs"] || !preg_match("~^sqlite_~", $C)) $J[$C] = $v;
        }
        return $J;
    }
    ...
    alter_table($R, $C, $p, $Pc, $qb, $mc, $d, $Ka, $tf)
    {
        ...
        if (!$ai) {
            ...
            if ($R != $C && !queries("ALTER TABLE " . table($R) . " RENAME TO " . table($C))) return
                false;
        } elseif (!recreate_table($R, $C, $c, $jf, $Pc)) return
            false;
    ...
    function
    recreate_table($R, $C, $p, $jf, $Pc, $w = array())
    {
        if ($R != "") {
            if (!$p) {
            ...
            foreach (indexes($R) as $Ld => $v) {
                ...
                if (!$Zb[$Ld]) {
                    if ($v["type"] != "PRIMARY" || !$If) $w[] = array($v["type"], $Ld, $f);
                }
            }
            foreach ($w
                     as $y => $X) {
            ...
            foreach (foreign_keys($R) as $Ld => $q) {
                ...
                if (!isset($Pc[" $Ld"])) $Pc[] = " " . format_foreign_key($q);
            }
            queries("BEGIN");
        }
        foreach ($p
        ...
        if (!queries("CREATE TABLE " . table($R != "" ? "adminer_$C" : $C) . " (\n" . implode(",\n", $p) . "\n)")) return
            false;
        if ($R != "") {
            if ($jf && !queries("INSERT INTO " . table("adminer_$C") . " (" . implode(", ", $jf) . ") SELECT " . implode(", ", array_map('idf_escape', array_keys($jf))) . " FROM " . table($R))) return
                false;
            $Lh = array();
    ...
    trigger($C)
    {
        ...
        $Je = $B[3];
        return
            array("Timing" => strtoupper($B[1]), "Event" => strtoupper($B[2]) . ($Je ? " OF" : ""), "Of" => ($Je[0] == '`' || $Je[0] == '"' ? idf_unescape($Je) : $Je), "Trigger" => $C, "Statement" => $B[4],);
    }
    ...
    $x = "sqlite";
    $Oh = array("integer" => 0, "real" => 0, "numeric" => 0, "text" => 0, "blob" => 0);
    $Vg = array_keys($Oh);
    $Vh = array();
    $Ve = array("=", "<", ">", "<=", ">=", "!=", "LIKE", "LIKE %%", "IN", "IS NULL", "NOT LIKE", "NOT IN", "IS NOT NULL", "SQL");
    $Xc = array("hex", "length", "lower", "round", "unixepoch", "upper");
    $cd = array("avg", "count", "count distinct", "group_concat", "max", "min", "sum");
    $ec = array(array(), array("integer|real|numeric" => "+/-", "text" => "||",));
}
$Wb["pgsql"] = "PostgreSQL";
if (isset($_GET["pgsql"])) {
    $Ef = array("PgSQL", "PDO_PgSQL");
    define("DRIVER", "pgsql");
        ...
        insertUpdate($R, $L, $Hf)
        {
            ...
            foreach ($L
                ...
                foreach ($O
                         as $y => $X) {
                    $Wh[] = "$y = $X";
                    if (isset($Hf[idf_unescape($y)])) $Z[] = "$y = $X";
                }
                if (!(($Z && queries("UPDATE " . table($R) . " SET " . implode(", ", $Wh) . " WHERE " . implode(" AND ", $Z)) && $g->affected_rows) || queries("INSERT INTO " . table($R) . " (" . implode(", ", array_keys($O)) . ") VALUES (" . implode(", ", $O) . ")"))) return
                    false;
            }
            return
                true;
    ...
    connect()
    {
        ...
        if ($g->connect($j[0], $j[1], $j[2])) {
            if ($g->server_info >= 9) {
                ...
                if ($g->server_info >= 9.2) {
                    $Vg['Strings'][] = "json";
                    $Oh["json"] = 4294967295;
                    if ($g->server_info >= 9.4) {
                        $Vg['Strings'][] = "jsonb";
                        $Oh["jsonb"] = 4294967295;
                    }
                }
...
" . ($C != "" ? "AND relname = " . q($C) : "ORDER BY c.oid")) as $K) $J[$K["Name"]] = $K;
        return ($C != "" ? $J[$C] : $J);
    }
...
ORDER BY a.attnum") as $K) {
    ...
    indexes($R, $h = null)
    {
        ...
        foreach (get_rows("SELECT relname, indisunique::int, indisprimary::int, indkey, indoption , (indpred IS NOT NULL)::int as indispartial FROM pg_index i, pg_class ci WHERE i.indrelid = $eh AND ci.oid = i.indexrelid", $h) as $K) {
            ...
            $J[$eg]["columns"] = array();
            foreach (explode(" ", $K["indkey"]) as $sd) $J[$eg]["columns"][] = $f[$sd];
            $J[$eg]["descs"] = array();
            foreach (explode(" ", $K["indoption"]) as $td) $J[$eg]["descs"][] = ($td & 1 ? '1' : null);
            $J[$eg]["lengths"] = array();
        }
        return $J;
    }
...
ORDER BY conkey, conname") as $K) {
    ...
    view($C)
    {
        global $g;
        return
            array("select" => trim($g->result("SELECT pg_get_viewdef(" . q($C) . ")")));
    }
        ...
        foreach ($p
            ...
            if (!$X) $c[] = "DROP $e"; else {
                ...
                if ($o[0] == "") $c[] = ($R != "" ? "ADD " : "  ") . implode($X); else {
                    if ($e != $X[0]) $Rf[] = "ALTER TABLE " . table($R) . " RENAME $e TO $X[0]";
                    $c[] = "ALTER $e TYPE$X[1]";
                    if (!$X[6]) {
                ...
                if ($o[0] != "" || $fi != "") $Rf[] = "COMMENT ON COLUMN " . table($R) . ".$X[0] IS " . ($fi != "" ? substr($fi, 9) : "''");
            }
        }
        $c = array_merge($c, $Pc);
        if ($R == "") array_unshift($Rf, "CREATE TABLE " . table($C) . " (\n" . implode(",\n", $c) . "\n)"); elseif ($c) array_unshift($Rf, "ALTER TABLE " . table($R) . "\n" . implode(",\n", $c));
        if ($R != "" && $R != $C) $Rf[] = "ALTER TABLE " . table($R) . " RENAME TO " . table($C);
        if ($R != "" || $qb != "") $Rf[] = "COMMENT ON TABLE " . table($C) . " IS " . q($qb);
        if ($Ka != "") {
        }
        foreach ($Rf
                 as $H) {
    ...
    alter_indexes($R, $c)
    {
        ...
        foreach ($c
                 as $X) {
            if ($X[0] != "INDEX") $i[] = ($X[2] == "DROP" ? "\nDROP CONSTRAINT " . idf_escape($X[1]) : "\nADD" . ($X[1] != "" ? " CONSTRAINT " . idf_escape($X[1]) : "") . " $X[0] " . ($X[0] == "PRIMARY" ? "KEY " : "") . "(" . implode(", ", $X[2]) . ")"); elseif ($X[2] == "DROP") $Xb[] = idf_escape($X[1]);
            else$Rf[] = "CREATE INDEX " . idf_escape($X[1] != "" ? $X[1] : uniqid($R . "_")) . " ON " . table($R) . " (" . implode(", ", $X[2]) . ")";
        }
        if ($i) array_unshift($Rf, "ALTER TABLE " . table($R) . implode(",", $i));
        if ($Xb) array_unshift($Rf, "DROP INDEX " . implode(", ", $Xb));
        foreach ($Rf
                 as $H) {
            if (!queries($H)) return
                false;
    ...
    move_tables($T, $li, $lh)
    {
        foreach (array_merge($T, $li) as $R) {
            $P = table_status($R);
            if (!queries("ALTER " . strtoupper($P["Engine"]) . " " . table($R) . " SET SCHEMA " . idf_escape($lh))) return
                false;
        }
        return
            true;
    ...
    {
        $J = array();
        foreach (get_rows("SELECT * FROM information_schema.triggers WHERE event_object_table = " . q($R)) as $K) $J[$K["trigger_name"]] = array($K["action_timing"], $K["event_manipulation"]);
        return $J;
    }
...
ORDER BY p.proname');
    ...
    {
        global $g;
        if (preg_match("~ rows=([0-9]+)~", $g->result("EXPLAIN SELECT * FROM " . idf_escape($S["Name"]) . ($Z ? " WHERE " . implode(" AND ", $Z) : "")), $dg)) return $dg[1];
        return
            false;
    }
...
AND typelem = 0");
    ...
    create_sql($R, $Ka)
    {
        ...
        if (!empty($Cg)) $J = implode("\n\n", $Cg) . "\n\n$J";
        foreach ($w
                 as $qd => $v) {
            switch ($v['type']) {
                case'UNIQUE':
                    $kg[] = "CONSTRAINT " . idf_escape($qd) . " UNIQUE (" . implode(', ', array_map('idf_escape', $v['columns'])) . ")";
                    break;
                case'PRIMARY':
                    $kg[] = "CONSTRAINT " . idf_escape($qd) . " PRIMARY KEY (" . implode(', ', array_map('idf_escape', $v['columns'])) . ")";
                    break;
            }
        }
        foreach ($Nc
                 as $Mc => $Lc) $kg[] = "CONSTRAINT " . idf_escape($Mc) . " $Lc[definition] " . ($Lc['deferrable'] ? 'DEFERRABLE' : 'NOT DEFERRABLE');
    ...
    $Vh = array();
    $Ve = array("=", "<", ">", "<=", ">=", "!=", "~", "!~", "LIKE", "LIKE %%", "ILIKE", "ILIKE %%", "IN", "IS NULL", "NOT LIKE", "NOT IN", "IS NOT NULL");
    $Xc = array("char_length", "lower", "round", "to_hex", "to_timestamp", "upper");
    $cd = array("avg", "count", "count distinct", "max", "min", "sum");
    $ec = array(array("char" => "md5", "date|time" => "now",), array("int|numeric|real|money" => "+/-", "date|time" => "+ interval/- interval", "char|text" => "||",));
}
$Wb["oracle"] = "Oracle";
if (isset($_GET["oracle"])) {
    $Ef = array("OCI8", "PDO_OCI");
...
ORDER BY 1") as $K) {
    ...
    fields($R)
    {
        ...
        foreach (get_rows("SELECT * FROM all_tab_columns WHERE table_name = " . q($R) . " ORDER BY column_id") as $K) {
            ...
            if ($Yd == ",") $Yd = $K["DATA_LENGTH"];
            $J[$K["COLUMN_NAME"]] = array("field" => $K["COLUMN_NAME"], "full_type" => $U . ($Yd ? "($Yd)" : ""), "type" => strtolower($U), "length" => $Yd, "default" => $K["DATA_DEFAULT"], "null" => ($K["NULLABLE"] == "Y"), "privileges" => array("insert" => 1, "select" => 1, "update" => 1),);
        }
        return $J;
    }
...
ORDER BY uc.constraint_type, uic.column_position", $h) as $K) {
    ...
    alter_table($R, $C, $p, $Pc, $qb, $mc, $d, $Ka, $tf)
    {
        ...
        foreach ($p
            ...
            if ($X) $c[] = ($R != "" ? ($o[0] != "" ? "MODIFY (" : "ADD (") : "  ") . implode($X) . ($R != "" ? ")" : ""); else$Xb[] = idf_escape($o[0]);
        }
        if ($R == "") return
...
');
    ...
    $Oh = array();
    $Vg = array();
    foreach (array('Numbers' => array("number" => 38, "binary_float" => 12, "binary_double" => 21), 'Date and time' => array("date" => 10, "timestamp" => 29, "interval year" => 12, "interval day" => 28), 'Strings' => array("char" => 2000, "varchar2" => 4000, "nchar" => 2000, "nvarchar2" => 4000, "clob" => 4294967295, "nclob" => 4294967295), 'Binary' => array("raw" => 2000, "long raw" => 2147483648, "blob" => 4294967295, "bfile" => 4294967296),) as $y => $X) {
        $Oh += $X;
        $Vg[$y] = array_keys($X);
    }
    $Vh = array();
    $Ve = array("=", "<", ">", "<=", ">=", "!=", "LIKE", "LIKE %%", "IN", "IS NULL", "NOT LIKE", "NOT REGEXP", "NOT IN", "IS NOT NULL", "SQL");
    $Xc = array("length", "lower", "round", "upper");
    $cd = array("avg", "count", "count distinct", "max", "min", "sum");
    $ec = array(array("date" => "current_date", "timestamp" => "current_timestamp",), array("number|float|double" => "+/-", "date|timestamp" => "+ interval/- interval", "char|clob" => "||",));
}
$Wb["mssql"] = "MS SQL";
if (isset($_GET["mssql"])) {
    $Ef = array("SQLSRV", "MSSQL", "PDO_DBLIB");
    define("DRIVER", "mssql");
        ...
        Min_SQL
    {
        ...
        insertUpdate($R, $L, $Hf)
        {
            foreach ($L
                ...
                if (!queries("MERGE " . table($R) . " USING (VALUES(" . implode(", ", $O) . ")) AS source (c" . implode(", c", range(1, count($O))) . ") ON " . implode(" AND ", $Z) . " WHEN MATCHED THEN UPDATE SET " . implode(", ", $Wh) . " WHEN NOT MATCHED THEN INSERT (" . implode(", ", array_keys($O)) . ") VALUES (" . implode(", ", $O) . ");")) return
                    false;
            }
            return
                true;
...
WHERE o.schema_id = SCHEMA_ID(" . q(get_schema()) . ") AND o.type IN ('S', 'U', 'V') AND o.name = " . q($R)) as $K) {
            ...
            $J[$K["name"]] = array("field" => $K["name"], "full_type" => $U . ($Yd ? "($Yd)" : ""), "type" => $U, "length" => $Yd, "default" => $K["default"], "null" => $K["is_nullable"], "auto_increment" => $K["is_identity"], "collation" => $K["collation_name"], "privileges" => array("insert" => 1, "select" => 1, "update" => 1), "primary" => $K["is_identity"],);
        }
        return $J;
    }
...
WHERE OBJECT_NAME(i.object_id) = " . q($R), $h) as $K) {
    ...
    alter_table($R, $C, $p, $Pc, $qb, $mc, $d, $Ka, $tf)
    {
        ...
        foreach ($p
            ...
            if (!$X) $c["DROP"][] = " COLUMN $e"; else {
                ...
                if ($o[0] == "") $c["ADD"][] = "\n  " . implode("", $X) . ($R == "" ? substr($Pc[$X[0]], 16 + strlen($X[0])) : ""); else {
                    unset($X[6]);
    ...
    alter_indexes($R, $c)
    {
        ...
        foreach ($c
                 as $X) {
            if ($X[2] == "DROP") {
                if ($X[0] == "PRIMARY") $Xb[] = idf_escape($X[1]); else$v[] = idf_escape($X[1]) . " ON " . table($R);
...
WHERE sys1.xtype = 'TR' AND sys2.name = " . q($R)) as $K) $J[$K["name"]] = array($K["Timing"], $K["Event"]);
        return $J;
    }
    ...
    $Vh = array();
    $Ve = array("=", "<", ">", "<=", ">=", "!=", "LIKE", "LIKE %%", "IN", "IS NULL", "NOT LIKE", "NOT IN", "IS NOT NULL");
    $Xc = array("len", "lower", "round", "upper");
    $cd = array("avg", "count", "count distinct", "max", "min", "sum");
    $ec = array(array("date|time" => "getdate",), array("int|decimal|real|float|money|datetime" => "+/-", "char|text" => "+",));
}
$Wb['firebird'] = 'Firebird (alpha)';
if (isset($_GET["firebird"])) {
    ...
    table_status($C = "", $Dc = false)
    {
        ...
        foreach ($Eb
                 as $v => $X) {
            $v = trim($v);
            $J[$v] = array('Name' => $v, 'Engine' => 'standard',);
            if ($C == $v) return $J[$v];
        }
        return $J;
    }
...
if (isset($_GET["simpledb"])) {
        ...
        Min_DB
        {
            ...
            query($H, $Ph = false)
            {
                ...
                if (preg_match('~^\s*SELECT\s+COUNT\(~i', $H)) {
                    $Zg = 0;
                    foreach ($I
                             as $Gd) $Zg += $Gd->Attribute->Value;
                    $I = array((object)array('Attribute' => array((object)array('Name' => 'Count', 'Value' => $Zg,))));
                }
                return
                    new
                    Min_Result($I);
        ...
        Min_Result
        {
            ...
            __construct($I)
            {
                foreach ($I
                         as $Gd) {
                    $K = array();
                    if ($Gd->Name != '') $K['itemName()'] = (string)$Gd->Name;
            ...
            fetch_field()
            {
                ...
                return (object)array('name' => $Md[$this->_offset++]);
            }
        }
    }
        ...
        Min_SQL
    {
        ...
        function
        _chunkRequest($nd, $ua, $F, $xc = array())
        {
            global $g;
            foreach (array_chunk($nd, 25) as $fb) {
                ...
                foreach ($fb
                    ...
                    foreach ($xc
                             as $y => $X) $of["Item.$s.$y"] = $X;
                }
                if (!sdb_request($ua, $of)) return
                    false;
            }
        ...
        {
            $J = array();
            if (preg_match_all("~itemName\(\) = (('[^']*+')+)~", $Sf, $he)) $J = array_map('idf_unescape', $he[1]); else {
                foreach (sdb_request_all('Select', 'Item', array('SelectExpression' => 'SELECT itemName() FROM ' . table($R) . $Sf . ($z ? " LIMIT 1" : ""))) as $Gd) $J[] = $Gd->Name;
            }
            return $J;
        }
        ...
        delete($R, $Sf, $z = 0)
        {
            return $this->_chunkRequest($this->_extractIds($R, $Sf, $z), 'BatchDeleteAttributes', array('DomainName' => $R));
        }
        ...
        update($R, $O, $Sf, $z = 0, $Ag = "\n")
        {
            ...
            foreach ($O
                     as $y => $X) {
                $y = idf_unescape($y);
                if ($X == "NULL" || ($t != "" && array($t) != $nd)) $Lb["Attribute." . count($Lb) . ".Name"] = $y;
                if ($X != "NULL") {
                    foreach ((array)$X
                             as $Id => $W) {
                        $yd["Attribute.$s.Name"] = $y;
                        $yd["Attribute.$s.Value"] = (is_array($X) ? $W : idf_unescape($W));
                        if (!$Id) $yd["Attribute.$s.Replace"] = "true";
                        $s++;
                    }
                }
            }
            $F = array('DomainName' => $R);
            return (!$yd || $this->_chunkRequest(($t != "" ? array($t) : $nd), 'BatchPutAttributes', $F, $yd)) && (!$Lb || $this->_chunkRequest($nd, 'BatchDeleteAttributes', $F, $Lb));
        }
    ...
    table_status($C = "", $Dc = false)
    {
        ...
        foreach (($C != "" ? array($C => true) : tables_list()) as $R => $U) {
            $K = array("Name" => $R, "Auto_increment" => "");
            if (!$Dc) {
                $ue = sdb_request('DomainMetadata', array('DomainName' => $R));
                if ($ue) {
                    foreach (array("Rows" => "ItemCount", "Data_length" => "ItemNamesSizeBytes", "Index_length" => "AttributeValuesSizeBytes", "Data_free" => "AttributeNamesSizeBytes",) as $y => $X) $K[$y] = (string)$ue->$X;
                }
            }
            if ($C != "") return $K;
            $J[$R] = $K;
    ...
    function
    hmac($Aa, $Eb, $y, $Wf = false)
    {
        $Ta = 64;
        if (strlen($y) > $Ta) $y = pack("H*", $Aa($y));
        $y = str_pad($y, $Ta, "\0");
    ...
    sdb_request($ua, $F = array())
    {
        ...
        $H = '';
        foreach ($F
                 as $y => $X) $H .= '&' . rawurlencode($y) . '=' . rawurlencode($X);
        $H = str_replace('%7E', '~', substr($H, 1));
        $H .= "&Signature=" . urlencode(base64_encode(hmac('sha1', "POST\n" . preg_replace('~^https?://~', '', $kd) . "\n/\n$H", $xg, true)));
        @ini_set('track_errors', 1);
        $Hc = @file_get_contents((preg_match('~^https?://~', $kd) ? $kd : "http://$kd"), false, stream_context_create(array('http' => array('method' => 'POST', 'content' => $H, 'ignore_errors' => 1,))));
        if (!$Hc) {
    ...
    function
    sdb_request_all($ua, $kh, $F = array(), $th = 0)
    {
        $J = array();
        ...
        do {
            ...
            if ($th && microtime(true) - $Sg > $th) return
                false;
    ...
    $x = "simpledb";
    $Ve = array("=", "<", ">", "<=", ">=", "!=", "LIKE", "LIKE %%", "IN", "IS NULL", "NOT LIKE", "IS NOT NULL");
    $Xc = array();
    $cd = array("count");
    $ec = array(array("json"));
}
$Wb["mongo"] = "MongoDB (beta)";
if (isset($_GET["mongo"])) {
    $Ef = array("mongo");
    define("DRIVER", "mongo");
        ...
        Min_SQL
    {
        ...
        select($R, $M, $Z, $ad, $af = array(), $z = 1, $E = 0, $Jf = false)
        {
            ...
            return
                new
                Min_Result($this->_conn->_db->selectCollection($R)->find(array(), $M)->sort($Mg)->limit(+$z)->skip($E * $z));
        }
    ...
    get_databases($Oc)
    {
        ...
        $J = array();
        $Ib = $g->_link->listDBs();
        foreach ($Ib['databases'] as $m) $J[] = $m['name'];
        return $J;
    }
    ...
    count_tables($l)
    {
        ...
        $J = array();
        foreach ($l
                 as $m) $J[$m] = count($g->_link->selectDB($m)->getCollectionNames(true));
        return $J;
    }
    ...
    indexes($R, $h = null)
    {
        ...
        foreach ($g->_db->selectCollection($R)->getIndexInfo() as $v) {
            $Ob = array();
            foreach ($v["key"] as $e => $U) $Ob[] = ($U == -1 ? '1' : null);
            $J[$v["name"]] = array("type" => ($v["name"] == "_id_" ? "PRIMARY" : ($v["unique"] ? "UNIQUE" : "INDEX")), "columns" => array_keys($v["key"]), "lengths" => array(), "descs" => $Ob,);
        }
        return $J;
    }
    ...
    alter_indexes($R, $c)
    {
        ...
        foreach ($c
                 as $X) {
            list($U, $C, $O) = $X;
            if ($O == "DROP") $J = $g->_db->command(array("deleteIndexes" => $R, "index" => $C)); else {
                $f = array();
                foreach ($O
                         as $e) {
                ...
                $J = $g->_db->selectCollection($R)->ensureIndex($f, array("unique" => ($U == "UNIQUE"), "name" => $C,));
            }
            if ($J['errmsg']) {
                $g->error = $J['errmsg'];
                return
...
if (isset($_GET["elastic"])) {
    ...
    if (function_exists('json_decode')) {
        ...
        Min_DB
        {
            ...
            function
            rootQuery($wf, $vb = array(), $ve = 'GET')
            {
                @ini_set('track_errors', 1);
                $Hc = @file_get_contents($this->_url . '/' . ltrim($wf, '/'), false, stream_context_create(array('http' => array('method' => $ve, 'content' => json_encode($vb), 'ignore_errors' => 1,))));
                if (!$Hc) {
        ...
        Min_SQL
    {
        ...
        select($R, $M, $Z, $ad, $af = array(), $z = 1, $E = 0, $Jf = false)
        {
            ...
            foreach ($Z
                ...
                if ($kb == "_id") $Eb["query"]["ids"]["values"][] = $X; elseif ($kb . $X != "") {
                    $oh = array("term" => array(($kb != "" ? $kb : "_all") => $X));
                    if ($Te == "=") $Eb["query"]["filtered"]["filter"]["and"][] = $oh; else$Eb["query"]["filtered"]["query"]["bool"]["must"][] = $oh;
                }
            }
            if ($Eb["query"] && !$Eb["query"]["filtered"]["query"] && !$Eb["query"]["ids"]) $Eb["query"]["filtered"]["query"] = array("match_all" => array());
            $Sg = microtime(true);
            $wg = $this->_conn->query($H, $Eb);
            if ($Jf) echo $b->selectQuery("$H: " . print_r($Eb, true), format_time($Sg));
            if (!$wg) return
                false;
            $J = array();
            foreach ($wg['hits']['hits'] as $jd) {
                $K = array();
                if ($M == array("*")) $K["_id"] = $jd["_id"];
                $p = $jd['_source'];
                if ($M != array("*")) {
                    $p = array();
                    foreach ($M
                             as $y) $p[$y] = $jd['fields'][$y];
                }
                foreach ($p
                         as $y => $X) {
                    if ($Eb["fields"]) $X = $X[0];
    ...
    tables_list()
    {
        global $g;
        $J = $g->query('_mapping');
        if ($J) $J = array_fill_keys(array_keys($J[$g->_db]["mappings"]), 'table');
        return $J;
    }
    ...
    {
        global $g;
        $wg = $g->query("_search?search_type=count", array("facets" => array("count_by_type" => array("terms" => array("field" => "_type",)))), "POST");
        $J = array();
        if ($wg) {
            foreach ($wg["facets"]["count_by_type"]["terms"] as $R) {
                $J[$R["term"]] = array("Name" => $R["term"], "Engine" => "table", "Rows" => $R["count"],);
                if ($C != "" && $C == $R["term"]) return $J[$C];
            }
        }
        return $J;
    }
    ...
    fields($R)
    {
        ...
        if ($I) {
            $ee = $I[$R]['properties'];
            if (!$ee) $ee = $I[$g->_db]['mappings'][$R]['properties'];
            if ($ee) {
                foreach ($ee
                         as $C => $o) {
    ...
    {
        global $g;
        return $g->rootQuery(urlencode($m), array(), 'PUT');
    }
    ...
    drop_tables($T)
    {
        ...
        $J = true;
        foreach ($T
                 as $R) $J = $J && $g->query(urlencode($R), array(), 'DELETE');
        return $J;
    }
...
if (!defined("DRIVER")) {
    ...
    if (extension_loaded("mysqli")) {
            ...
            MySQLi
        {
            ...
            function
            connect($N = "", $V = "", $G = "", $k = null, $Af = null, $Lg = null)
            {
    ...
    connect()
    {
        ...
        $J = $g->error;
        if (function_exists('iconv') && !is_utf8($J) && strlen($sg = iconv("windows-1250", "utf-8", $J)) > strlen($J)) $J = $sg;
        return $J;
    }
    ...
    db_collation($m, $mb)
    {
        ...
        if (preg_match('~ COLLATE ([^ ]+)~', $i, $B)) $J = $B[1]; elseif (preg_match('~ CHARACTER SET ([^ ]+)~', $i, $B)) $J = $mb[$B[1]][-1];
        return $J;
    }
    ...
    rename_database($C, $d)
    {
        ...
        if (create_database($C, $d)) {
            $fg = array();
            foreach (tables_list() as $R => $U) $fg[] = table($R) . " TO " . idf_escape($C) . "." . table($R);
    ...
    {
        $fg = array();
        foreach (array_merge($T, $li) as $R) $fg[] = table($R) . " TO " . idf_escape($lh) . "." . table($R);
        return
    ...
    copy_tables($T, $li, $lh)
    {
        ...
        foreach ($li
            ...
            $C = ($lh == DB ? table("copy_$R") : idf_escape($lh) . "." . table($R));
            $ki = view($R);
            if (!queries("DROP VIEW IF EXISTS $C") || !queries("CREATE VIEW $C AS $ki[select]")) return
                false;
        }
        return
            true;
...
', "<p><input type='submit' value='" . 'Login' . "'>\n", checkbox("auth[permanent]", 1, $_COOKIE["adminer_permanent"], 'Permanent login') . "\n";
    ...
    selectLinks($ch, $O = "")
    {
        ...
        if (support("table") || support("indexes")) $be["table"] = 'Show structure';
        if (support("table")) {
            if (is_view($ch)) $be["view"] = 'Alter view'; else$be["create"] = 'Alter table';
        }
        if ($O !== null) $be["edit"] = 'New item';
        foreach ($be
                 as $y => $X) echo " <a href='" . h(ME) . "$y=" . urlencode($ch["Name"]) . ($y == "edit" ? $O : "") . "'" . bold(isset($_GET[$y])) . ">$X</a>";
        echo "\n";
    }
    ...
    selectVal($X, $_, $o, $if)
    {
        ...
        if (preg_match('~blob|bytea|raw|file~', $o["type"]) && !is_utf8($X)) $J = "<i>" . lang(array('%d byte', '%d bytes'), strlen($if)) . "</i>";
        if (preg_match('~json~', $o["type"])) $J = "<code class='jush-js'>$J</code>";
    ...
    tableIndexesPrint($w)
    {
        ...
        foreach ($w
            ...
            ksort($v["columns"]);
            $Jf = array();
            foreach ($v["columns"] as $y => $X) $Jf[] = "<i>" . h($X) . "</i>" . ($v["lengths"][$y] ? "(" . $v["lengths"][$y] . ")" : "") . ($v["descs"][$y] ? " DESC" : "");
    ...
    selectSearchProcess($p, $w)
    {
        ...
        foreach ($w
                 as $s => $v) {
            if ($v["type"] == "FULLTEXT" && $_GET["fulltext"][$s] != "") $J[] = "MATCH (" . implode(", ", array_map('idf_escape', $v["columns"])) . ") AGAINST (" . q($_GET["fulltext"][$s]) . (isset($_GET["boolean"][$s]) ? " IN BOOLEAN MODE" : "") . ")";
        }
        foreach ((array)$_GET["where"] as $X) {
            if ("$X[col]$X[val]" != "" && in_array($X["op"], $this->operators)) {
                ...
                if (preg_match('~IN$~', $X["op"])) {
                    ...
                    $sb .= " " . ($pd != "" ? $pd : "(NULL)");
                } elseif ($X["op"] == "SQL") $sb = " $X[val]";
                elseif ($X["op"] == "LIKE %%") $sb = " LIKE " . $this->processInput($p[$X["col"]], "%$X[val]%");
                elseif ($X["op"] == "ILIKE %%") $sb = " ILIKE " . $this->processInput($p[$X["col"]], "%$X[val]%");
                elseif (!preg_match('~NULL$~', $X["op"])) $sb .= " " . $this->processInput($p[$X["col"]], $X["val"]);
    ...
    function
    selectQueryBuild($M, $Z, $ad, $af, $z, $E)
    {
        return "";
    }
    ...
    editFunctions($o)
    {
        ...
        foreach ($ec
            ...
            if (!$y || (!isset($_GET["call"]) && (isset($_GET["select"]) || where($_GET)))) {
                foreach ($Xc
                         as $yf => $X) {
                    if (!$yf || preg_match("~$yf~", $o["type"])) $J .= "/$X";
                }
                if ($y && !preg_match('~set|blob|bytea|raw|file~', $o["type"])) $J .= "/SQL";
            }
        }
    ...
    editInput($R, $o, $Ia, $Y)
    {
        if ($o["type"] == "enum") return (isset($_GET["select"]) ? "<label><input type='radio'$Ia value='-1' checked><i>" . 'original' . "</i></label> " : "") . ($o["null"] ? "<label><input type='radio'$Ia value=''" . ($Y !== null || isset($_GET["select"]) ? "" : " checked") . "><i>NULL</i></label> " : "") . enum_input("radio", $Ia, $o, $Y, 0);
        return "";
    }
    ...
    dumpTable($R, $Wg, $Fd = 0)
    {
        if ($_POST["format"] != "sql") {
            echo "\xef\xbb\xbf";
            if ($Wg) dump_csv(array_keys(fields($R)));
        } else {
            if ($Fd == 2) {
                $p = array();
                foreach (fields($R) as $C => $o) $p[] = idf_escape($C) . " $o[full_type]";
                $i = "CREATE TABLE " . table($R) . " (" . implode(", ", $p) . ")";
            } else$i = create_sql($R, $_POST["auto_increment"]);
            set_utf8mb4($i);
            if ($Wg && $i) {
                if ($Wg == "DROP+CREATE" || $Fd == 1) echo "DROP " . ($Fd == 2 ? "VIEW" : "TABLE") . " IF EXISTS " . table($R) . ";\n";
                if ($Fd == 1) $i = remove_definer($i);
                echo "$i;\n\n";
            }
        }
    }
            ...
            if ($I) {
                ...
                while ($K = $I->$Fc()) {
                    ...
                    if ($_POST["format"] != "sql") {
                        ...
                        if (!$yd) $yd = "INSERT INTO " . table($R) . " (" . implode(", ", array_map('idf_escape', $Md)) . ") VALUES";
                        foreach ($K
                                 as $y => $X) {
                            $o = $p[$y];
                        ...
                        $sg = ($je ? "\n" : " ") . "(" . implode(",\t", $K) . ")";
                        if (!$Wa) $Wa = $yd . $sg; elseif (strlen($Wa) + 4 + strlen($sg) + strlen($Yg) < $je) $Wa .= ",$sg";
                        else {
                            echo $Wa . $Yg;
                            $Wa = $yd . $sg;
                        }
    ...
    dumpHeaders($md, $ye = false)
    {
        ...
        if ($lf == "gz") ob_start('ob_gzencode', 1e6);
        return $_c;
    }
...
';
        if ($xe == "auth") {
                ...
                foreach ($Fg
                    ...
                    foreach ($di
                        ...
                        if ($G !== null) {
                            ...
                            foreach (($Ib ? array_keys($Ib) : array("")) as $m) echo "<a href='" . h(auth_url($ii, $N, $V, $m)) . "'>($Wb[$ii]) " . h($V . ($N != "" ? "@$N" : "") . ($m != "" ? " - $m" : "")) . "</a><br>\n";
                        }
                    }
                }
            }
            ...
            echo '<script type="text/javascript" src="', h(preg_replace("~\\?.*~", "", ME)) . "?file=jush.js&amp;version=4.3.1", '"></script>
';
            if (support("sql")) {
                echo '<script type="text/javascript">
';
...
';
        ...
        echo "<span title='" . 'database' . "'>DB</span>: " . ($l ? "<select name='db'$Gb>" . optionlist(array("" => "") + $l, DB) . "</select>" : '<input name="db" value="' . h(DB) . '" autocapitalize="off">'), "<input type='submit' value='" . 'Use' . "'" . ($l ? " class='hidden'" : "") . ">\n";
...
Adminer);
if ($b->operators === null) $b->operators = $Ve;
function
page_header($vh, $n = "", $Va = array(), $wh = ""){
    global $ca, $ia, $b, $Wb, $x;
    page_headers();
    if (is_ajax() && $n) {
        page_messages($n);
...
';
    if ($b->head()) {
        echo '<link rel="shortcut icon" type="image/x-icon" href="', h(preg_replace("~\\?.*~", "", ME)) . "?file=favicon.ico&amp;version=4.3.1", '">
            ...
            if ($Va === false) echo "$N\n"; else {
                ...
                if (is_array($Va)) {
                    ...
                    foreach ($Va
                        ...
                        if ($Nb != "") echo "<a href='" . h(ME . "$y=") . urlencode(is_array($X) ? $X[0] : $X) . "'>$Nb</a> &raquo; ";
                    }
                }
                echo "$vh\n";
            }
...
';
        ...
        int32($_e)
        {
            while ($_e >= 2147483648) $_e -= 4294967296;
            while ($_e <= -2147483649) $_e += 4294967296;
            return (int)$_e;
        }function
        long2str($W, $ni)
        {
        ...
        decrypt_string($Ug, $y)
        {
            ...
            return
                long2str($W, true);
        }$g = '';$gd = $_SESSION["token"];if (!$gd) $_SESSION["token"] = rand(1, 1e6);$Ah = get_token();$zf = array();if ($_COOKIE["adminer_permanent"]) {
        ...
        add_invalid_login()
        {
            ...
            if ($Ad) {
                foreach ($Ad
                         as $Bd => $X) {
                    if ($X[0] < $sh) unset($Ad[$Bd]);
                }
            }
            $_d =& $Ad[$b->bruteForceKey()];
            if (!$_d) $_d = array($sh + 30 * 60, 0);
            $_d[1]++;
            $Dg = serialize($Ad);
            rewind($Vc);
            fwrite($Vc, $Dg);
            ...
            if ($Fe > 0) auth_error(lang(array('Too many unsuccessful logins, try again in %d minute.', 'Too many unsuccessful logins, try again in %d minutes.'), ceil($Fe / 60)));
            ...
            if (count($_POST) == 1 || DRIVER != $ii || SERVER != $N || $_GET["username"] !== $V || DB != $m) redirect(auth_url($ii, $N, $V, $m));
        } elseif ($_POST["logout"]) {
            if ($gd && !verify_token()) {
        ...
        unset_permanent()
        {
            ...
            foreach ($zf
                     as $y => $X) {
                list($ii, $N, $V, $m) = array_map('base64_decode', explode("-", $y));
                if ($ii == DRIVER && $N == SERVER && $V == $_GET["username"] && $m == DB) unset($zf[$y]);
            }
            cookie("adminer_permanent", implode(" ", $zf));
        }function
        auth_error($n)
        {
            ...
            if (isset($_GET["username"])) {
                ...
                if (($_COOKIE[$Gg] || $_GET[$Gg]) && !$gd) $n = 'Session expired, please login again.'; else {
                    add_invalid_login();
                    $G = get_password();
                    if ($G !== null) {
                        if ($G === false) $n .= '<br>' . sprintf('Master password expired. <a href="https://www.adminer.org/en/extension/" target="_blank">Implement</a> %s method to make it permanent.', '<code>permanentLogin()</code>');
                        set_password(DRIVER, SERVER, $_GET["username"], null);
                    }
                    unset_permanent();
                }
            }
            if (!$_COOKIE[$Gg] && $_GET[$Gg] && ini_bool("session.use_only_cookies")) $n = 'Session support must be enabled.';
        ...
        Min_Driver($g);if (!is_object($g) || ($ce = $b->login($_GET["username"], get_password())) !== true) auth_error((is_string($g) ? h($g) : (is_string($ce) ? $ce : 'Invalid credentials.')));if ($Ja && $_POST["token"]) $_POST["token"] = $Ah;$n = '';if ($_POST) {
            if (!verify_token()) {
                $vd = "max_input_vars";
                $ne = ini_get($vd);
            ...
            for ($s = 0; (!$z || $s < $z) && ($K = $I->fetch_row()); $s++) {
                if (!$s) {
                    ...
                    for ($Hd = 0; $Hd < count($K); $Hd++) {
                        ...
                        $bf = $o->orgname;
                        $J[$o->table] = $cf;
                        if ($df && $x == "sql") $be[$Hd] = ($C == "table" ? "table=" : ($C == "possible_keys" ? "indexes=" : null)); elseif ($cf != "") {
                            if (!isset($w[$cf])) {
                                $w[$cf] = array();
                ...
                foreach ($K
                         as $y => $X) {
                    if ($X === null) $X = "<i>NULL</i>"; elseif ($Sa[$y] && !is_utf8($X)) $X = "<i>" . lang(array('%d byte', '%d bytes'), strlen($X)) . "</i>";
                    elseif (!strlen($X)) $X = "&nbsp;";
                    else {
                        $X = h($X);
                        if ($Oh[$y] == 254) $X = "<code>$X</code>";
                    }
                    if (isset($be[$y]) && !$f[$be[$y]]) {
                        if ($df && $x == "sql") {
                            ...
                            foreach ($w[$be[$y]] as $kb => $Hd) $_ .= "&where" . urlencode("[" . bracket_escape($kb) . "]") . "=" . urlencode($K[$Hd]);
                        }
                        $X = "<a href='" . h(ME . $_) . "'>$X</a>";
                    }
                    echo "<td>$X";
        ...
        edit_type($y, $o, $mb, $Rc = array())
        {
            ...
            if ($U && !isset($Oh[$U]) && !isset($Rc[$U])) array_unshift($Vg, $U);
            if ($Rc) $Vg['Foreign keys'] = $Rc;
            echo
...
', (support("comment") ? "<td id='label-comment'" . ($rb ? "" : " class='hidden'") . ">" . 'Comment' : "");
        }echo '<td>', "<input type='image' class='icon' name='add[" . (support("move_col") ? 0 : count($p)) . "]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=plus.gif&amp;version=4.3.1' alt='+' title='" . 'Add next' . "'>", '<script type="text/javascript">row_count = ', count($p), ';</script>
</thead>
...
', ($U == "PROCEDURE" ? "<td>" . html_select("fields[$s][inout]", explode("|", $wd), $o["inout"]) : ""), '<th>';if ($Rb) {
...
';edit_type("fields[$s]", $o, $mb, $Rc);if($U == "TABLE"){echo '<td>', checkbox("fields[$s][null]", 1, $o["null"], "", "", "block", "label-null"), '<td><label class="block"><input type="radio" name="auto_increment_col" value="', $s, '"';if ($o["auto_increment"]) {
            echo ' checked';
        }?> onclick="var field = this.form['
        fields[' + this.value + '][field]
        ']; if (!field.value) { field.value = '
        id
        '; field.onchange(); }" aria-labelledby="label-ai"></label><td><?php
            echo
...
', (support("comment") ? "<td" . ($rb ? "" : " class='hidden'") . "><input name='fields[$s][comment]' value='" . h($o["comment"]) . "' maxlength='" . ($g->server_info >= 5.5 ? 1024 : 255) . "' aria-labelledby='label-comment'>" : "");}echo "<td>", (support("move_col") ? "<input type='image' class='icon' name='add[$s]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=plus.gif&amp;version=4.3.1' alt='+' title='" . 'Add next' . "' onclick='return !editingAddRow(this, 1);'>&nbsp;" . "<input type='image' class='icon' name='up[$s]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=up.gif&amp;version=4.3.1' alt='^' title='" . 'Move up' . "' onclick='return !editingMoveRow(this, 1);'>&nbsp;" . "<input type='image' class='icon' name='down[$s]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=down.gif&amp;version=4.3.1' alt='v' title='" . 'Move down' . "' onclick='return !editingMoveRow(this, 0);'>&nbsp;" : ""), ($ef == "" || support("drop_col") ? "<input type='image' class='icon' name='drop_col[$s]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=cross.gif&amp;version=4.3.1' alt='x' title='" . 'Remove' . "' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">" : ""), "\n";}}function
        ...
        grant($Yc, $Mf, $f, $Pe)
        {
            if (!$Mf) return
                true;
            if ($Mf == array("ALL PRIVILEGES", "GRANT OPTION")) return ($Yc == "GRANT" ? queries("$Yc ALL PRIVILEGES$Pe WITH GRANT OPTION") : queries("$Yc ALL PRIVILEGES$Pe") && queries("$Yc GRANT OPTION$Pe"));
            return
                queries("$Yc " . preg_replace('~(GRANT OPTION)\\([^)]*\\)~', '\\1', implode("$f, ", $Mf) . $f) . $Pe);
        }function
        drop_create($Xb, $i, $Yb, $ph, $ac, $A, $se, $qe, $re, $Me, $Ce)
        {
            if ($_POST["drop"]) query_redirect($Xb, $A, $se); elseif ($Me == "") query_redirect($i, $A, $re);
            elseif ($Me != $Ce) {
                $Bb = queries($i);
        ...
        create_routine($og, $K)
        {
            ...
            foreach ($p
                     as $o) {
                if ($o["field"] != "") $O[] = (preg_match("~^($wd)\$~", $o["inout"]) ? "$o[inout] " : "") . idf_escape($o["field"]) . process_type($o, "CHARACTER SET");
            }
        ...
        tar_file($Ic, $zh)
        {
            ...
            $eb = 8 * 32;
            for ($s = 0; $s < strlen($J); $s++) $eb += ord($J[$s]);
            $J .= sprintf("%06o", $eb) . "\0 ";
        ...
        }$Qe = "RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";class
        TmpFile
        {
            var $handler;
            var $size;
        ...
        }$oc = "'(?:''|[^'\\\\]|\\\\.)*'";$wd = "IN|OUT|INOUT";if (isset($_GET["select"]) && ($_POST["edit"] || $_POST["clone"]) && !$_POST["save"]) $_GET["edit"] = $_GET["select"];if (isset($_GET["callf"])) $_GET["call"] = $_GET["callf"];if (isset($_GET["function"])) $_GET["procedure"] = $_GET["function"];if(isset($_GET["download"])){
            $a = $_GET["download"];
            $p = fields($a);
            ...
            foreach (table_status('', true) as $R => $S) {
                ...
                foreach ($b->foreignKeys($R) as $X) {
                    if (!$X["db"]) {
                        $Ud = $Pa;
                        if ($fh[$R][1] || $fh[$X["table"]][1]) $Ud = min(floatval($fh[$R][1]), floatval($fh[$X["table"]][1])) - 1; else$Pa -= .1;
                        while ($Wd[(string)$Ud]) $Ud -= .0001;
            ...
            foreach ($ug
                ...
                foreach ((array)$bg[$C] as $mh => $cg) {
                    foreach ($cg
                        ...
                        $s = 0;
                        foreach ($f
                                 as $lh) echo "\n<div class='references' title='" . h($mh) . "' id='refd$Ud-" . ($s++) . "' style='left: $Vd" . "em; top: " . $R["fields"][$lh]["pos"] . "em; height: 1.25em; background: url(" . h(preg_replace("~\\?.*~", "", ME)) . "?file=arrow.gif) no-repeat right center;&amp;version=4.3.1'><div style='height: .5em; border-bottom: 1px solid Gray; width: " . (-$Vd) . "em;'></div></div>";
                    }
                }
                echo "\n</div>\n";
            }
...
';
            ...
            if ($_POST && !$n) {
                $zb = "";
                foreach (array("output", "format", "db_style", "routines", "events", "table_style", "auto_increment", "triggers", "data_style") as $y) $zb .= "&$y=" . urlencode($_POST[$y]);
                ...
                foreach ((array)$l
                    ...
                    if ($g->select_db($m)) {
                        if ($Dd && preg_match('~CREATE~', $Wg) && ($i = $g->result("SHOW CREATE DATABASE " . idf_escape($m), 1))) {
                            set_utf8mb4($i);
                            if ($Wg == "DROP+CREATE") echo "DROP DATABASE IF EXISTS " . idf_escape($m) . ";\n";
                            echo "$i;\n";
                        }
                        if ($Dd) {
                            if ($Wg) echo
                        ...
                        if ($_POST["table_style"] || $_POST["data_style"]) {
                            ...
                            foreach (table_status('', true) as $C => $S) {
                                ...
                                if ($R || $Eb) {
                                    ...
                                    if (is_view($S)) $li[] = $C; elseif ($Eb) {
                                        $p = fields($C);
                                        $b->dumpData($C, $_POST["data_style"], "SELECT *" . convert_fields($p, $p) . " FROM " . table($C));
                                    }
                                    if ($Dd && $_POST["triggers"] && $R && ($Lh = trigger_sql($C, $_POST["table_style"]))) echo "\nDELIMITER ;;\n$Lh\nDELIMITER ;\n";
                                    if ($_c == "tar") {
                                        ob_end_flush();
                            ...
                            foreach ($li
                                     as $ki) $b->dumpTable($ki, $_POST["table_style"], 1);
                            if ($_c == "tar") echo
                            pack("x512");
                        }
                    }
                }
                if ($Dd) echo "-- " . $g->result("SELECT NOW()") . "\n";
                exit;
            }
...
';
            ...
            if (DB != "") {
                ...
                foreach ($ih
                    ...
                    if ($U !== null && !preg_match('~table~i', $U)) $li .= "$Jf\n"; else
...
';
            ...
            $Yc = $I;
            if (!$I) $I = $g->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");
            echo "<form action=''><p>\n";
            hidden_fields_get();
            ...
            if (!$n && $_POST) {
                ...
                if (is_string($H)) {
                            ...
                            if (!$Tc && $Vc && !feof($Vc)) $H .= fread($Vc, 1e5); else {
                                ...
                                if ($Tc && rtrim($Tc) != $Mb) {
                                    ...
                                    if ($x == "sqlite" && preg_match("~^$Og*+ATTACH\\b~i", $Qf, $B)) {
                                        ...
                                        $Sg = microtime(true);
                                        if ($g->multi_query($Qf) && is_object($h) && preg_match("~^$Og*+USE\\b~i", $Qf)) $h->query($Qf);
                                        do {
                                            $I = $g->store_result();
                                            ...
                                            if ($g->error) {
                                                ...
                                                if (!$_POST["only_errors"]) echo "<p class='message' title='" . h($g->info) . "'>" . lang(array('Query executed OK, %d row affected.', 'Query executed OK, %d rows affected.'), $g->affected_rows) . "$sh\n";
                                            }
                                            $Sg = microtime(true);
                                        } while ($g->next_result());
                                    }
                    ...
                    if ($lc) echo "<p class='message'>" . 'No commands to execute.' . "\n"; elseif ($_POST["only_errors"]) {
...
';
            ...
            if (!isset($_GET["import"])) {
                $Qf = $_GET["sql"];
                if ($_POST) $Qf = $_POST["query"]; elseif ($_GET["history"] == "all") $Qf = $hd;
                elseif ($_GET["history"] != "") $Qf = $hd[$_GET["history"]][0];
                echo "<p>";
...
';
            ...
            foreach ($p
                     as $C => $o) {
                if (!isset($o["privileges"][$Wh ? "update" : "insert"]) || $b->fieldName($o) == "") unset($p[$C]);
            }
            if ($_POST && !$n && !isset($_GET["select"])) {
                $A = $_POST["referer"];
                if ($_POST["insert"]) $A = ($Wh ? null : $_SERVER["REQUEST_URI"]); elseif (!preg_match('~^.+&select=.+$~', $A)) $A = ME . "select=" . urlencode($a);
                $w = indexes($a);
                $Rh = unique_array($_GET["where"], $w);
                $Tf = "\nWHERE $Z";
                if (isset($_POST["delete"])) queries_redirect($A, 'Item has been deleted.', $Vb->delete($a, $Tf, !$Rh)); else {
                    $O = array();
                    foreach ($p
                             as $C => $o) {
                        $X = process_input($o);
            ...
            if ($_POST["save"]) $K = (array)$_POST["fields"]; elseif ($Z) {
                ...
                foreach ($p
                    ...
                    if (isset($o["privileges"]["select"])) {
                        ...
                        if ($x == "sql" && preg_match("~enum|set~", $o["type"])) $Fa = "1*" . idf_escape($C);
            ...
            edit_form($a, $p, $K, $Wh);
        }elseif(isset($_GET["create"])){$a = $_GET["create"];$rf = array();foreach (array('HASH', 'LINEAR HASH', 'KEY', 'LINEAR KEY', 'RANGE', 'LIST') as $y) $rf[$y] = $y;$ag = referencable_primary($a);$Rc = array();foreach ($ag
                                                                                                                                                                                                                                 as $dh => $o) $Rc[str_replace("`", "``", $dh) . "`" . str_replace("`", "``", $o["field"])] = $dh;$gf = array();$S = array();if ($a != "") {
            $gf = fields($a);
            $S = table_status($a);
            if (!$S) $n = 'No tables.';
            ...
            if ($_POST["drop"]) queries_redirect(substr(ME, 0, -1), 'Table has been dropped.', drop_tables(array($a))); else {
                ...
                foreach ($K["fields"] as $y => $o) {
                    ...
                    if ($o["field"] != "") {
                        ...
                        if ($y == $K["auto_increment_col"]) $o["auto_increment"] = true;
                        $Of = process_field($o, $Mh);
                        $Ca[] = array($o["orig"], $Of, $_a);
                        if ($Of != process_field($ff, $ff)) {
                            $p[] = array($o["orig"], $Of, $_a);
                            if ($o["orig"] != "" || $_a) $ai = true;
                        }
                        if ($q !== null) $Pc[idf_escape($o["field"])] = ($a != "" && $x != "sqlite" ? "ADD" : " ") . format_foreign_key(array('table' => $Rc[$o["type"]], 'source' => array($o["field"]), 'target' => array($Mh["field"]), 'on_delete' => $o["on_delete"],));
                ...
                } elseif (support("partitioning") && preg_match("~partitioned~", $S["Create_options"])) $tf .= "\nREMOVE PARTITIONING";
        ...
        }page_header(($a != "" ? 'Alter table' : 'Create table'), $n, array("table" => $a), h($a));if (!$_POST) {
        ...
        }$mb = collations();$nc = engines();foreach ($nc
                                                     as $mc) {
...
';
    ...
    if (preg_match('~MyISAM|M?aria' . ($g->server_info >= 5.6 ? '|InnoDB' : '') . '~i', $S["Engine"])) $rd[] = "FULLTEXT";
    if (preg_match('~MyISAM|M?aria' . ($g->server_info >= 5.7 ? '|InnoDB' : '') . '~i', $S["Engine"])) $rd[] = "SPATIAL";
    $w = indexes($a);
    $Hf = array();
    if ($x == "mongo") {
        $Hf = $w["_id_"];
    ...
    if ($_POST && !$n && !$_POST["add"] && !$_POST["drop_col"]) {
        ...
        foreach ($w
                 as $C => $wc) $c[] = array($wc["type"], $C, "DROP");
    ...
    if ($_POST["add"]) {
        foreach ($K["indexes"] as $y => $v) {
            if ($v["columns"][count($v["columns"])] != "") $K["indexes"][$y]["columns"][] = "";
        }
        $v = end($K["indexes"]);
        if ($v["type"] || array_filter($v["columns"], 'strlen')) $K["indexes"][] = array("columns" => array(1 => ""));
    }
    if (!$K) {
        foreach ($w
                 as $y => $v) {
    ...
    <tr>
        ...
        <th>
            <noscript><input type='image' class='icon' name='add[0]'
                             src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=plus.gif&amp;version=4.3.1' alt='+'
                             title='Add next'></noscript>
            &nbsp;
    </thead>
    <?php
    if ($Hf) {
...
';
        ...
        if ($_POST["drop"]) {
            ...
            if (DB != "") {
                ...
                foreach ($l
                         as $m) {
                    if (count($l) == 1 || $m != "") {
                        if (!create_database($m, $K["collation"])) $Xg = false;
                        $Sd = $m;
                    }
                }
                restart_session();
    ...
    $mb = collations();
    $C = DB;
    if ($_POST) $C = $K["name"]; elseif (DB != "") $K["collation"] = db_collation(DB, $mb);
    elseif ($x == "sql") {
...
';
    ...
    if ($_POST && !$n) {
        ...
        if ($_POST["drop"]) query_redirect("DROP SCHEMA " . idf_escape($_GET["ns"]), $_, 'Schema has been dropped.'); else {
            $C = trim($K["name"]);
            $_ .= urlencode($C);
            if ($_GET["ns"] == "") query_redirect("CREATE SCHEMA " . idf_escape($C), $_, 'Schema has been created.'); elseif ($_GET["ns"] != $C) query_redirect("ALTER SCHEMA " . idf_escape($_GET["ns"]) . " RENAME TO " . idf_escape($C), $_, 'Schema has been altered.');
            else
                redirect($_);
        }
    }
...
';
    ...
    foreach ($og["fields"] as $s => $o) {
        if (substr($o["inout"], -3) == "OUT") $kf[$s] = "@" . idf_escape($o["field"]) . " AS " . idf_escape($o["field"]);
        if (!$o["inout"] || substr($o["inout"], 0, 2) == "IN") $pd[] = $s;
    }
    if (!$n && $_POST) {
        $Xa = array();
        foreach ($og["fields"] as $y => $o) {
            if (in_array($y, $pd)) {
                $X = process_input($o);
                if ($X === false) $X = "''";
                if (isset($kf[$y])) $g->query("SET @" . idf_escape($o["field"]) . " = $X");
            }
        ...
        if (!$g->multi_query($H)) echo "<p class='error'>" . error() . "\n"; else {
            ...
            do {
                $I = $g->store_result();
                if (is_object($I)) select($I, $h); else
            ...
            if ($kf) select($g->query("SELECT " . implode(", ", $kf)));
        }
    }
    echo '
<form action="" method="post">
...
';
    ...
    if ($_POST && !$n && !$_POST["add"] && !$_POST["change"] && !$_POST["change-js"]) {
        ...
        if (!$_POST["drop"]) {
            ...
            ksort($K["source"]);
            $lh = array();
            foreach ($K["source"] as $y => $X) $lh[$y] = $K["target"][$y];
            $K["target"] = $lh;
        }
        if ($x == "sqlite") queries_redirect($A, $pe, recreate_table($a, $a, array(), array(), array(" $C" => ($_POST["drop"] ? "" : " " . format_foreign_key($K))))); else {
            $c = "ALTER TABLE " . table($a);
            $Xb = "\nDROP " . ($x == "sql" ? "FOREIGN KEY " : "CONSTRAINT ") . idf_escape($C);
            if ($_POST["drop"]) query_redirect($c . $Xb, $A, $pe); else {
...
';
    ...
    if ($_POST && !$n) {
        ...
        if (!$_POST["drop"] && $a == $C && $x != "sqlite" && $U == "VIEW" && $hf == "VIEW") query_redirect(($x == "mssql" ? "ALTER" : "CREATE OR REPLACE") . " VIEW " . table($C) . $Fa, $A, $pe); else {
...
';
    ...
    $K = $_POST;
    if ($_POST && !$n) {
        if ($_POST["drop"]) query_redirect("DROP EVENT " . idf_escape($aa), substr(ME, 0, -1), 'Event has been dropped.'); elseif (in_array($K["INTERVAL_FIELD"], $zd) && isset($Tg[$K["STATUS"]])) {
...
';
    ...
    if ($_POST && !process_fields($K["fields"]) && !$n) {
        ...
        drop_create("DROP $og " . idf_escape($da), create_routine($og, $K), "DROP $og " . idf_escape($K["name"]), create_routine($og, array("name" => $nh) + $K), "DROP $og " . idf_escape($nh), substr(ME, 0, -1), 'Routine has been dropped.', 'Routine has been altered.', 'Routine has been created.', $da, $K["name"]);
    }
...
';
    ...
    if ($_POST && !$n) {
        $_ = substr(ME, 0, -1);
        $C = trim($K["name"]);
        if ($_POST["drop"]) query_redirect("DROP SEQUENCE " . idf_escape($fa), $_, 'Sequence has been dropped.'); elseif ($fa == "") query_redirect("CREATE SEQUENCE " . idf_escape($C), $_, 'Sequence has been created.');
        elseif ($fa != $C) query_redirect("ALTER SEQUENCE " . idf_escape($fa) . " RENAME TO " . idf_escape($C), $_, 'Sequence has been altered.');
        else
            redirect($_);
    }
...
';
    ...
    if ($_POST && !$n) {
        $_ = substr(ME, 0, -1);
        if ($_POST["drop"]) query_redirect("DROP TYPE " . idf_escape($ga), $_, 'Type has been dropped.'); else
...
';} elseif (isset($_GET["user"])) {
    ...
    foreach (get_rows("SHOW PRIVILEGES") as $K) {
        foreach (explode(",", ($K["Privilege"] == "Grant option" ? "" : $K["Context"])) as $xb) $Mf[$xb][$K["Privilege"]] = $K["Comment"];
    }
    ...
    $Mf["Columns"] = array();
    foreach (array("Select", "Insert", "Update", "References") as $X) $Mf["Columns"][$X] = $Mf["Tables"][$X];
    unset($Mf["Server Admin"]["Usage"]);
    foreach ($Mf["Tables"] as $y => $X) unset($Mf["Databases"][$y]);
    $Be = array();
    if ($_POST) {
        foreach ($_POST["objects"] as $y => $X) $Be[$X] = (array)$Be[$X] + (array)$_POST["grants"][$y];
    }
    $Zc = array();
    $Ne = "";
    if (isset($_GET["host"]) && ($I = $g->query("SHOW GRANTS FOR " . q($ha) . "@" . q($_GET["host"])))) {
        while ($K = $I->fetch_row()) {
            if (preg_match('~GRANT (.*) ON (.*) TO ~', $K[0], $B) && preg_match_all('~ *([^(,]*[^ ,(])( *\\([^)]+\\))?~', $B[1], $he, PREG_SET_ORDER)) {
                foreach ($he
                         as $X) {
                    if ($X[1] != "USAGE") $Zc["$B[2]$X[2]"][$X[1]] = true;
                    if (preg_match('~ WITH GRANT OPTION~', $K[0])) $Zc["$B[2]$X[2]"]["GRANT OPTION"] = true;
                }
            }
            if (preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~", $K[0], $B)) $Ne = $B[1];
        }
    }
    if ($_POST && !$n) {
        $Oe = (isset($_GET["host"]) ? q($ha) . "@" . q($_GET["host"]) : "''");
        if ($_POST["drop"]) query_redirect("DROP USER $Oe", ME . "privileges=", 'User has been dropped.'); else {
            ...
            if (!$n) {
                if ($Oe != $De) {
                    ...
                    $n = !$Bb;
                } elseif ($vf != $Ne) queries("SET PASSWORD FOR $De = " . q($vf));
            }
            if (!$n) {
                $lg = array();
                foreach ($Be
                         as $Ie => $Yc) {
                    if (isset($_GET["grant"])) $Yc = array_filter($Yc);
                    $Yc = array_keys($Yc);
                    if (isset($_GET["grant"])) $lg = array_diff(array_keys(array_filter($Be[$Ie], 'strlen')), $Yc); elseif ($Oe == $De) {
            ...
            if (!$n && isset($_GET["host"])) {
                if ($Oe != $De) queries("DROP USER $Oe"); elseif (!isset($_GET["grant"])) {
                    foreach ($Zc
                             as $Ie => $lg) {
                        if (preg_match('~^(.+)(\\(.*\\))?$~U', $Ie, $B)) grant("REVOKE", array_keys($lg), $B[2], " ON $B[1] FROM $De");
                    }
                }
            }
            queries_redirect(ME . "privileges=", (isset($_GET["host"]) ? 'User has been altered.' : 'User has been created.'), !$n);
            if ($Bb) $g->query("DROP USER $De");
        }
    }
    ...
    foreach (array("" => "", "Server Admin" => 'Server', "Databases" => 'Database', "Tables" => 'Table', "Columns" => 'Column', "Procedures" => 'Routine',) as $xb => $Nb) {
        foreach ((array)$Mf[$xb] as $Lf => $qb) {
            ...
            foreach ($Zc
                ...
                $Y = $Yc[strtoupper($Lf)];
                if ($xb == "Server Admin" && $Ie != (isset($Zc["*.*"]) ? "*.*" : ".*")) echo "<td>&nbsp;"; elseif (isset($_GET["grant"])) echo "<td><select name=$C><option><option value='1'" . ($Y ? " selected" : "") . ">" . 'Grant' . "<option value='0'" . ($Y == "0" ? " selected" : "") . ">" . 'Revoke' . "</select>";
                else
...
';
    ...
    foreach (process_list() as $s => $K) {
        ...
        foreach ($K
                 as $y => $X) echo "<td>" . (($x == "sql" && $y == "Info" && preg_match("~Query|Killed~", $K["Command"]) && $X != "") || ($x == "pgsql" && $y == "current_query" && $X != "<IDLE>") || ($x == "oracle" && $y == "sql_text" && $X != "") ? "<code class='jush-$x'>" . shorten_utf8($X, 100, "</code>") . ' <a href="' . h(ME . ($K["db"] != "" ? "db=" . urlencode($K["db"]) . "&" : "") . "sql=" . urlencode($X)) . '">' . 'Clone' . '</a>' : nbsp($X));
        echo "\n";
    }
    echo '</table>
<script type=\'text/javascript\'>tableCheck();</script>
...
';
    ...
    if (!$fe) echo "<p class='message'>" . 'No rows.' . "\n"; else {
        echo "<table cellspacing='0'>\n";
        foreach ($fe[0] as $y => $X) {
    ...
    foreach ($p
        ...
        if (isset($o["privileges"]["select"]) && $C != "") {
            ...
            if (is_shortable($o)) $rh = $b->selectLengthProcess();
        }
        $mg += $o["privileges"];
    }
    ...
    if ($_POST && !$n) {
        ...
        foreach ((array)$Uh
                 as $y => $X) {
            if (in_array(idf_escape($y), $M)) unset($Uh[$y]);
        }
        if ($_POST["export"]) {
            cookie("adminer_import", "output=" . urlencode($_POST["output"]) . "&format=" . urlencode($_POST["format"]));
            dump_headers($a);
            $b->dumpTable($a, "");
            if (!is_array($_POST["check"]) || $Uh === array()) $H = "SELECT $Wc$pi$bd"; else {
                $Qh = array();
                foreach ($_POST["check"] as $X) $Qh[] = "(SELECT" . limit($Wc, "\nWHERE " . ($Z ? implode(" AND ", $Z) . " AND " : "") . where_check($X, $p) . $bd, 1) . ")";
        ...
        if (!$b->selectEmailProcess($Z, $Rc)) {
            if ($_POST["save"] || $_POST["delete"]) {
                ...
                if ($_POST["clone"] && $I && $ya == 1) {
                    $Td = last_id();
                    if ($Td) $pe = sprintf('Item%s has been inserted.', " $Td");
                }
            ...
            elseif (!preg_match('~~u', $Hc)) $n = 'File must be in UTF-8 encoding.';
            else {
                ...
                foreach ($he[0] as $y => $X) {
                    ...
                    if (!$y && !array_diff($ie[1], $nb)) {
                        ...
                        $O = array();
                        foreach ($ie[1] as $s => $kb) $O[idf_escape($nb[$s])] = ($kb == "" && $p[$nb[$s]]["null"] ? "NULL" : q(str_replace('""', '"', preg_replace('~^"|"$~', '', $kb))));
                        $L[] = $O;
                    }
                }
    ...
    if (isset($mg["insert"]) || !support("table")) {
        $O = "";
        foreach ((array)$_GET["where"] as $X) {
            if (count($Rc[$X["col"]]) == 1 && ($X["op"] == "=" || (!$X["op"] && !preg_match('~[_%]~', $X["val"])))) $O .= "&set" . urlencode("[" . bracket_escape($X["col"]) . "]") . "=" . urlencode($X["val"]);
        }
    }
    $b->selectLinks($S, $O);
    if (!$f && support("table")) echo "<p class='error'>" . 'Unable to select the table' . ($p ? "." : ": " . error()) . "\n"; else {
        ...
        if ($yb) $yg[] = substr($yb, 2);
        $I = $Vb->select($a, $yg, $Z, $ad, $af, $z, $E, true);
        if (!$I) echo "<p class='error'>" . error() . "\n"; else {
            if ($x == "mssql" && $E) $I->seek($z * $E);
            $kc = array();
            echo "<form action='' method='post' enctype='multipart/form-data'>\n";
            $L = array();
            while ($K = $I->fetch_assoc()) {
                if ($E && $x == "oracle") unset($K["RNUM"]);
                $L[] = $K;
            }
            if ($_GET["page"] != "last" && +$z && $ad && $Cd && $x == "sql") $Uc = $g->result(" SELECT FOUND_ROWS()");
            if (!$L) echo "<p class='message'>" . 'No rows.' . "\n"; else {
                ...
                foreach ($L[0] as $y => $X) {
                    if ($y != $Ke) {
                        ...
                        if ($C != "") {
                            ...
                            if (!$X["fun"]) echo '<a href="#fieldset-search" onclick="selectSearch(\'' . h(js_escape($y)) . '\'); return false;" title="' . 'Search' . '" class="text jsonly"> =</a>';
                            echo "</span>";
                        }
                        $Xc[$y] = $X["fun"];
                        next($M);
                ...
                if ($_GET["modify"]) {
                    foreach ($L
                             as $K) {
                        foreach ($K
                                 as $y => $X) $Zd[$y] = max($Zd[$y], min(40, strlen(utf8_decode($X))));
                    }
                }
                ...
                foreach ($b->rowDescriptions($L, $Rc) as $_e => $K) {
                    ...
                    if (!$Rh) {
                        $Rh = array();
                        foreach ($L[$_e] as $y => $X) {
                            if (!preg_match('~^(COUNT\\((\\*|(DISTINCT )?`(?:[^`]|``)+`)\\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\\(`(?:[^`]|``)+`\\))$~', $y)) $Rh[$y] = $X;
                        }
                    }
                    $Sh = "";
                    foreach ($Rh
                    ...
                    foreach ($K
                        ...
                        if (isset($Ae[$y])) {
                            $o = $p[$y];
                            if ($X != "" && (!isset($kc[$y]) || $kc[$y] != "")) $kc[$y] = (is_mail($X) ? $Ae[$y] : "");
                            $_ = "";
                            if (preg_match('~blob|bytea|raw|file~', $o["type"]) && $X != "") $_ = ME . 'download=' . urlencode($a) . '&field=' . urlencode($y) . $Sh;
                            if (!$_ && $X !== null) {
                                foreach ((array)$Rc[$y] as $q) {
                                    if (count($Rc[$y]) == 1 || end($q["source"]) == $y) {
                                        $_ = "";
                                        foreach ($q["source"] as $s => $Ng) $_ .= where_link($s, $q["target"][$s], $L[$_e][$Ng]);
                            ...
                            if ($y == "COUNT(*)") {
                                ...
                                foreach ((array)$_GET["where"] as $W) {
                                    if (!array_key_exists($W["col"], $Rh)) $_ .= where_link($s++, $W["col"], $W["val"], $W["op"]);
                                }
                                foreach ($Rh
                                         as $Id => $W) $_ .= where_link($s++, $Id, $W);
                            }
            ...
            if (($L || $E) && !is_ajax()) {
                $uc = true;
                if ($_GET["page"] != "last") {
                    if (!+$z) $Uc = count($L); elseif ($x != "sql" || !$Cd) {
                        $Uc = ($Cd ? false : found_rows($S, $Z));
                        if ($Uc < max(1e4, 2 * ($E + 1) * $z)) $Uc = reset(slow_query(count_rows($a, $Z, $Cd, $ad))); else$uc = false;
                    }
                }
                if (+$z && ($Uc === false || $Uc > $z || $E)) {
                    ...
                    if ($x != "simpledb") {
                        ...
                        for ($s = max(1, $E - 4); $s < min($ke, $E + 5); $s++) echo
                        pagination($s, $E);
                        if ($ke > 0) {
...
';
    ...
    if (!$hi) echo "<p class='message'>" . 'No rows.' . "\n"; else {
        echo "<table cellspacing='0'>\n";
        foreach ($hi
                 as $y => $X) {
    ...
    if ($_GET["script"] == "db") {
        ...
        foreach (table_status() as $C => $S) {
            ...
            if (!is_view($S)) {
                foreach (array("Engine", "Collation") as $y) json_row("$y-$C", nbsp($S[$y]));
    ...
    if ($jh && !$n && !$_POST["search"]) {
        $I = true;
        $pe = "";
        if ($x == "sql" && count($_POST["tables"]) > 1 && ($_POST["drop"] || $_POST["truncate"] || $_POST["copy"])) queries("SET foreign_key_checks = 0");
        if ($_POST["truncate"]) {
        ...
        } elseif (!$_POST["tables"]) $pe = 'No tables.';
        elseif ($I = queries(($_POST["optimize"] ? "OPTIMIZE" : ($_POST["check"] ? "CHECK" : ($_POST["repair"] ? "REPAIR" : "ANALYZE"))) . " TABLE " . implode(", ", array_map('idf_escape', $_POST["tables"])))) {
            while ($K = $I->fetch_assoc()) $pe .= "<b>" . h($K["Table"]) . "</b>: " . h($K["Msg_text"]) . "<br>";
        }
        ...
        if ($_GET["ns"] !== "") {
            ...
            if (support("event")) {
                ...
                if ($L) {
                    ...
                    if ($sc && $sc != "ON") echo "<p class='error'><code class='jush-sqlset'>event_scheduler</code>: " . h($sc) . "\n";
                }
